...
export const EditCompanyAdminAction = async (data: EditCompanyAdminInterface) => {
  const session = await auth();
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/user/profile`, {
    method: "PUT",
    headers: {
...
export const EditCompanyInformationAction = async (data: EditCompanyInformationInterface) => {
  const session = await auth();
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/user/company-profile`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      //@ts-ignore
