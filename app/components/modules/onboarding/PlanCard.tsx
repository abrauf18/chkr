import React from 'react';
import checkIcon from "@/app/assets/icons/checkmark-circle.svg";
import Image from 'next/image';

interface PlanCardProps {
  title: string;
  price: number;
  features: string[];
  timePeriod: string;
}

const PlanCard: React.FC<PlanCardProps> = ({
  title,
  price,
  features,
  timePeriod,
}) => {

  const formattedPrice = price.toFixed(2);

  return (
    <div className="rounded-2xl overflow-hidden w-full mx-auto mb-4 hover:border-2 hover:border-primary focus:border-2 focus:border-primary">
      <div className='flex justify-between p-4'>
        <div className='flex-col'>
          <h3 className="text-lg font-semibold">{title}</h3>
          <span className='text-gray-500 text-sm'>select one subscription plan</span>
        </div>
        <div className='flex flex-col'>
          <span className="text-lg font-semibold">$ {formattedPrice} USD</span>
          <span className='text-gray-500 text-sm'>per {timePeriod}</span>
        </div>
      </div>

      <ul className="list-disc space-y-2 text-gray-700 p-4">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center">
            <Image src={checkIcon} alt="Check Icon" className="w-4 h-4 mr-2" />
            <span className='text-gray-400 text-sm'>
              Feature {index + 1}: {feature}
            </span>
          </li>
        ))}
      </ul>
    </div>
  );
};

export default PlanCard;
