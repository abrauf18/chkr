// components/GoogleMapsGeofencing.tsx
  ...
  useEffect(() => {
    const initMap = () => {
      if (mapRef.current) {
        ...
        const logCoordinates = (path: google.maps.MVCArray<google.maps.LatLng>) => {
        ...
        google.maps.event.addListener(drawingManagerInstance, 'overlaycomplete', (event: google.maps.drawing.OverlayCompleteEvent) => {
          if (event.type === google.maps.drawing.OverlayType.POLYGON) {
            const newPolygon = event.overlay as google.maps.Polygon;
            const path = newPolygon.getPath();
            logCoordinates(path);
            google.maps.event.addListener(path, 'set_at', () => logCoordinates(path));
            google.maps.event.addListener(path, 'insert_at', () => logCoordinates(path));
          }
        });
  ...
  return (
    <>
      ...
      <div className="flex flex-col w-full ">
        ...
        <p className='text-left text-xs md:text-sm mb-4'>Please set the boundary on map.</p>
